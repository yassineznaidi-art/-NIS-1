...
class MemorySubstrateV02:
    ...
    def _semantic_score_tfidf(self, query: str, docs: List[str]) -> List[float]:
        """Use sklearn TF-IDF cosine as semantic proxy. Returns list per doc."""
        try:
    ...
    def retrieve(self, query: str, intent_frame: Dict, deliberation_level: str, project_scope: Optional[str] = None) -> Dict:
        ...
        # Compute lexical + semantic per candidate
        docs=[str(c.value)[:2000] + " " + c.key for c in candidates]
        semantic_scores=self._semantic_score_tfidf(query, docs)
        ...
        threshold=0.08 if level_num>=3 else 0.12
        filtered=[h for h in scored if h.relevance >= threshold]
        # If filtered too few and query is Helios etc., keep top 3 anyway to ensure recall
        ...
        # Build provenance-enhanced result
        ...
        return {
            ...
            "source": "hybrid: lexical+semantic+metadata+freshness+rerank",
            "memory_store": "S3+S4+S5+RERANK",
            "retrieval_path": "N5→S3/S4/S5→TFIDF→rerank(S6)→freshness→budget"
        }
